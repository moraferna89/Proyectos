...
Implementación propia con pandas/numpy — sin dependencias externas de TA.
"""
...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """
    Recibe un DataFrame OHLCV (columnas: time, open, high, low, close, volume)
    y retorna el mismo DataFrame con columnas de indicadores añadidas.
    ...
    if dropped > 0:
        logger.debug("Eliminadas %d filas con NaN (warm-up de indicadores).", dropped)

    logger.info("Indicadores calculados. Filas resultantes: %d", len(df))
    return df
